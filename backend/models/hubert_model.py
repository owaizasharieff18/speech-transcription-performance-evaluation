...
class HuBERTBaseModel(BaseSTTModel):
    """
    Adapter 3: Meta HuBERT (facebook/hubert-large-ls960-ft).
    Lazy-loads weights on first transcribe request.
    """
    def __init__(self, model_id: str = "ts3", name: str = "HuBERT-Base (TS3)"):
        super().__init__(
            model_id=model_id,
            name=name,
            description="Meta HuBERT - Hidden-Unit BERT Acoustic CTC Model"
        )
        self.pretrained_path = "facebook/hubert-large-ls960-ft"
